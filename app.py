...
# --- Password Protection ---
AUTHORIZED_PASSWORDS = st.secrets.get("AUTHORIZED_PASSWORDS", ["default_password"])
def check_password():
...
elevenlabs_models = get_elevenlabs_model_list()
...
def play_sound(text, voice_id):
    try:
        ...
        audio_b64 = base64.b64encode(audio_bytes_io.read()).decode()
        audio_html = f"""
        <audio autoplay>
...
def display_chat():
    ...
    if st.session_state.get("messages"):
        for msg in st.session_state.messages[1:]:
            if msg["role"] == "system":
                continue
            role_label = "🧑‍💼 Interviewer" if msg["role"] == "assistant" else f"🙋 {st.session_state.get('interview_name', 'User')}"
            content = escape(msg["content"]).replace("\n", "<br>")
            inner += f"<p><strong>{role_label}:</strong><br>{content}</p><hr>"
    ...
    """
    chat_placeholder.html(chat_html, height=420, scrolling=False)
...
if "config_initialized" not in st.session_state:
    ...
    st.session_state.update({
        ...
        "story_stages":{
            "Moment": False, "Details": False, "Realisation": False, "Change": False, "Resolution": False
        },
...
with tab1:
    st.title("🗣️ Interviewer")
    name = st.text_input("Enter your Name")
    st.session_state["interview_name"] = name if name else "User"

    # --- Interview Initialization ---
    def init_interview():
        if not st.session_state.get("interview_name") or st.session_state["interview_name"] == "User":
            st.warning("Please enter your name to start the interview.")
            return
        st.session_state["interview_start_time"] = datetime.now()
        for key in ["messages", "transcript", "analysis", "view_analysis", "interview_ended", "last_audio_data"]:
    ...
    if st.button("🎤 Begin Interview"):
        ...
        if st.session_state.get("messages") and st.session_state["messages"][-1]["role"] == "assistant":
            if st.session_state.get("talk_back"):
                play_sound(st.session_state["messages"][-1]["content"], st.session_state.get("voice_id", "pNInz6obpgDQGcFmaJgB"))
    
    # --- Chat Display and Message Processing ---
    display_chat()
    ...
    def process_message(user_input):
        ...
        # 1. Append user message and update display
        ...
        # 3. Perform background analysis (Steering & Safeguarding)
        ...
        # 5. Append assistant reply and update display again
        st.session_state.messages.append({"role": "assistant", "content": reply})
        display_chat()

        # 6. Play sound if checkbox is ticked
        if st.session_state.get("talk_back"):
            play_sound(reply, st.session_state.get("voice_id", "pNInz6obpgDQGcFmaJgB"))

    # --- Chat Input Controls ---
    if st.session_state.get("messages"):
        st.checkbox("🔊 Interviewer should talk back", key="talk_back", value=True)
        
        # Audio input (using an if-block to prevent rerun issues)
        if st.session_state.get("last_audio_data"):
            with st.spinner("Transcribing..."):
                try:
                    audio_bytes_io = BytesIO(st.session_state["last_audio_data"])
                    ...
                    st.session_state["last_audio_data"] = None # Clear after use
                    st.rerun() # Re-run to clear the audio input widget
                except Exception as e:
                    st.error(f"Transcription failed: {e}")
                    st.session_state["last_audio_data"] = None
        
        audio_data = st.audio_recorder("🎙️ Speak your answer instead")
        if audio_data and audio_data != st.session_state.get("last_audio_data_check"):
             st.session_state["last_audio_data"] = audio_data
             st.session_state["last_audio_data_check"] = audio_data
             st.rerun()

        # Text input (using a simple if-block)
...
with tab2:
    if 'analysis' not in st.session_state:
        ...
        user_transcript = st.file_uploader("Upload your own transcript:", type=["txt"], accept_multiple_files=False)
        if user_transcript:
        ...
        if 'narrative' in st.session_state:
            ...
            story_option = st.selectbox("What type of Story would you like to generate?", ("Adult's Story", "Children's Story", "EYFS Story"))
            if voice_options: # Check if voice_options is not empty
                selected_voice_name = st.selectbox("Select Voice:", list(voice_options.keys()), key="voice_name")
                ...
                if story_option == "Adult's Story":
                    with st.spinner("Writing your story..."):
                        ...
                        try:
                            ...
                            if voice_options: # Only attempt to play if voices exist
                                play_sound(st.session_state['adult_story'], voice_id=voice_id)
                            st.success("Story Created! - Enjoy!")
                        except Exception as e:
                            st.error("Story Generation Failed - Please try again...")
                elif story_option == "Children's Story":
                    with st.spinner("Writing your story..."):
                        child_story_context = ""
                        ...
                        try:
                            ...
                            if voice_options: # Only attempt to play if voices exist
                                play_sound(st.session_state['child_story'], voice_id=voice_id)
                            st.success("Story Created! - Enjoy!")
                        except Exception as e:
                            st.error("Story Generation Failed - Please try again...")
                elif story_option == "EYFS Story":
                    with st.spinner("Writing your story..."):
                        eyfs_story_context = ""
                        ...
                        try:
                            ...
                            if voice_options: # Only attempt to play if voices exist
                                play_sound(st.session_state['eyfs_story'], voice_id=voice_id)
                            st.success("Story Created! - Enjoy!")
                        except Exception as e:
                            st.error("Story Generation Failed - Please try again...")
...
with tab3:
    ...
    if uploaded_prereq_files:
        ...
        current_uploaded_names = {file.name for file in uploaded_prereq_files}
        st.session_state.file_titles = {
    ...
    if elevenlabs_models:
        st.selectbox("Select Model:", elevenlabs_models, key="TTS_model")
    ...
    with tab1s:
        ...
        st.multiselect("Select prerequisite files for Interviewer", options=prereq_titles, key="interview_selected_files")
    with tab2s:
        st.subheader("Assistant Bot Settings")
        with st.expander("Expand to view steering prompts:"):
            st.selectbox("Select a steering model", chat_models, key="steering_model")
            st.text_area("Interview Steering Prompt:", key="steering_prompt", height=350)
        with st.expander("Expand to view safeguarding prompt"):
            st.selectbox("Select a safeguarding model", chat_models, key="safeguarding_model")
            st.text_area("Interview Safeguarding Prompt:", key="safeguarding_prompt", height=350)
    with tab3s:
        st.subheader("Analysis Settings")
        with st.expander("Expand to edit Analysis prompts:"):
            st.text_area("Analysis System Configuration Prompt:", key="analysis_system_prompt", height=350)
            st.text_area("Analysis Initiation Prompt:", key="analysis_init_prompt")
        st.selectbox("Select a model", chat_models, key="analysis_model")
        st.multiselect("Select prerequisite files for Analysis", options=prereq_titles, key="analysis_selected_files")
    with tab4s:
        ...
        with tab2as:
            ...
            st.multiselect("Select prerequisite files for Adult Story Generation", options=prereq_titles, key="adult_story_files")
        with tab3cs:
            ...
            st.multiselect("Select prerequisite files for Child Story Generation", options=prereq_titles, key="child_story_files")
        with tab4es:
            ...
            st.multiselect("Select prerequisite files for EYFS Story Generation", options=prereq_titles, key="eyfs_story_files")